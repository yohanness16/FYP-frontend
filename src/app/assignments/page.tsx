  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      >
        <div>
          ...
          <p style={{ fontSize: 11.5, color: "var(--text-3)", marginTop: 2 }}>
            Admin · refreshes every 20s · {lastUpdated.toLocaleTimeString()}
          </p>
        </div>
        <button
          type="button"
      ...
      >
        ...
        <div>
          <p style={{ fontSize: 13, fontWeight: 500, color: "var(--neon)" }}>
            Active trips
          </p>
          <p style={{ fontSize: 12, color: "var(--text-2)", marginTop: 2 }}>
            Data from <code>/assignments/active</code>. Use &quot;Force End&quot; to complete a
            stuck assignment.
          </p>
        </div>
      </div>
      ...
      {assignments.length === 0 ? (
        <div className="card" style={{ textAlign: "center", padding: "4rem 2rem" }}>
          ...
          <p style={{ fontSize: 13, color: "var(--text-3)" }}>
            Start one via <code>POST /assignments/start</code> (driver, vehicle, route) or from
            your driver app when wired.
          </p>
        </div>
      ) : (
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr className="tbl-head">
                ...
                <th className="tbl-th">Route</th>
                <th className="tbl-th">Driver</th>
                <th className="tbl-th">Started</th>
                <th className="tbl-th">Elapsed</th>
                <th className="tbl-th" style={{ textAlign: "right" }}>
                  Action
                </th>
              </tr>
            </thead>
            <tbody>
              {assignments.map((a) => {
                ...
                const plate = a.vehicle_plate ?? v?.plate_number ?? `#${a.vehicle_id}`;
                const routeLabel = a.route_number
                  ? `${a.route_number}`
                  : r
                    ? `${r.route_number} · ${r.name || r.origin || ""}`
                    : `Route #${a.route_id}`;
                const driverLabel = a.driver_username ?? `#${a.driver_id}`;
                return (
                  <tr key={a.id} className="tbl-row">
                    <td className="tbl-td">
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <Bus size={14} color="var(--neon)" />
                        <span
                          style={{
                            fontWeight: 500,
